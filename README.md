...
>
>예를 들어 인기 있는 사이트는 여러 서버에 중복되어 있어서 서로 다른 ip주소를 가진다. 
>
...
>>- TCP fast retransmit
>>  - 똑같은 세그먼트에 대해 ACK이 3번 들어왔다면, 세그먼트가 손실되었다는 것을 의미한다.
...
>
>처리량이 R근처의 양만큼 동작하는 것은 처리량 관점에서는 이상적이지만 지연 관점에서는 이상적이지 않다. 또한 재전송이 없다.
>
...
>
>netwokr service model로 답을 할 수있다.
>
...
>
> **기존 전통 방법(라우터별 제어)** : 위 그림은 라우팅 알고리즘들이 모든 라우터 각각에서 동작하는 경우를 나타낸다. 포워딩과 라우팅 기능이 모두 각각의 라우터에 들어있어서 모든 라우터는 서로 구성요소들은 통신하여 자신의 포워딩 테이블을 수정하게 된다. 
>
...
>
>**라우터**는 **네트워크 계층** 필드 값에 근거하여 포워딩을 결정한다.
>
...
>
>옛날방식으로는 IP address 도착지를 기반으로 포워딩 했지만 요즘은 헤더 필드 값에 근거하여 포워딩을 한다.
>
>**match plus action**
>
>match - 목적지 IP 주소를 찾음
...
>
>기존의 라우터에서는 라우팅 프로토콜을 실행하고 라우팅 테이블과 연결된 링크 상태 정보를 유지 관리하며 라우터의 포워딩 테이블을 계산 하였지만 
>
>**SDN 라우터**에서 라우팅 프로세서는 원격 컨트롤러와 통신하여 원격 컨트롤러에서 계산된 포워딩 테이블 항목을 수신하고 라우터의 입력포트에 항목을 설치한다.
>
...
>
>지정된 전달 테이블 항목을 찾을 때 대상 주소, 대상 주소와 일치하는 **가장 긴 주소 접두사**를 사용한다.
>
>즉 위에서 말하는 검색은 포워딩 테이블을 검색하여 가장 긴 프리픽스와 일치 하는것을 찾는 것이다.(겹치는 것이 많은 프리픽스)
>
>![image-20201105142459086](README.assets/image-20201105142459086.png)
>
>>검색을 통해 패킷의 출력포트가 결정되면 패킷을 스위칭 구조로 보낼 수 있다.
>
>#### **Switching fabrics**(스위칭 구조)
>
>>- 검색을 통해 패킷의 출력 포트가 결정되면 패킷을 스위칭 구조로 보낼수 있다. 만약 다른 포트가 스위칭 구조(switch fabric) 을 사용한다면 패킷이 스위칭 구조에 들어가는 것을 일시적으로 차단 가능하다. 차단된 패킷은 입력포트 큐에 대기한 후에 스위칭 구조에 들어간다. 
>
>>  따라서 패킷의 차단, 대기열 처리, 스케줄링 같은 역할도 해낸다. 입력포트에서 검색이 가장 중요한 기능이지만 이외에 중요한 기능을 알아보자
>
>>  물리 및 링크 계층 프로세싱 해야함
>>
>>  패킷의 버전번호, 체크섬 및 TTL 필드를 확인하고 두 필드를 다시 사용한다.
>>
>>  네트워크 관리에 사용되는 카운터(수신 된 데이터 그램 수) 를 업데이트해야 한다.
>
>스위칭 구조를 통해 패킷이 입력 포트에서 출력 포트로 실제로 스위칭(즉, 전달)되므로 스위칭 구조는 라우터의 핵심이다.
>
>​	패킷을 인풋 버퍼에서 적절한 출력 버퍼로 이송시킴
>
>​	witching rate: 패킷이 인풋에서 아웃풋으로 전송될 수 있는 속도
>
...
>>
>> 두번째 사진처럼 앞의 패킷이 움직이지 않아서 기다리는 경우도 있다. 
>
...
>
>>- **scheduling discipline**은 전송되길 기다리는 (queued)데이타그램 사이에서 최고의 효율을 내는 것을 고른다.
>
>>버퍼가 커지면 라우터의 메모리가 소모될 수 있으며 패킷을 저장 할 수 있는 메모리가 없을때 패킷 손실 발생한다.
>
>#### packet scheduling
>
>##### FIFO: 큐처럼 먼저 들어온 것이 먼저 나간다.
>
>##### 우선순위 큐잉: 우선순위 큐가 두 개 있어 우선순위가 높은 것을 먼저 보냄
>
>##### 라운드 로빈(RR): 우선순위가 같은 큐를 번갈아가며 수행
>
>##### WFQ(weighted Fair Queueing):  RR이랑 비슷하지만 각 큐마다 가중치(weight)만큼 서비스 시간을 보장받는다. 
>
...
>- BGP: Border Gateway Protocol      -> 외부 라우팅 프로토콜
>
...
>
>    이 필드 값은 IP 데이터그램에서 데이터 부분이 전달될 목적지의 전송 계층의 특정 프로토콜을 명시한다. 값 6은 데이터 부분을 TCP로 , 값 17은 UDP로 데  이터로 전달 하라는 것이다.
>
...
>
>- 데이터(페이로드) : 목적지에 전달하기 위해 전송 계층 세그먼트를 포함하지만 ICMP 메세지와 같은 다른
>
>    유형의 데이터를 담기도 하다.
...
>
> - ex)1600바이트 데이터그램을 500바이트의 MTU를 가지는 링크로 전송한다고 생각하자. 원래 데이터그램에 식별번호 291이 포함되었다고 가정하자. 얼마나 많은 단편들이 생성되는가? 단편화에 관련되어 생성된 IP 데이터그램의 다양한 필드들의 값은 무엇인가?
>
> - answer) 입력패킷 : 20+1580 (바이트)
>
...
>
>- 그림을 보면, 하나의 라우터가 7개의 호스트를 연결한다.
>
> - 왼쪽 3개의 호스트와 연결된 라우터 인터페이스 모두 223.1.1.xxx 형식의 IP 주소를 갖는다.
>   
>   - 즉 동일한 왼쪽 24비트를 사용한다.
>   
> - 또한 3개의 인터페이스가 중계하는 라우터 없이 하나의 네트워크에 서로 연결되어 있다.
>
> - 이 네트워크는 이더넷 LAN으로 상호 연결되고 이 경우 인터페이스는 **이더넷 허브나 이더넷 스위치** 또는 무선 액세스 포인터로 상호 연결된다.
>
> - 세 호스트들의 인터페이스들과 하나의 라우터 인터페이스로 연결된 네트워크는 **서브넷**을 구성한다고 말한다. 
>
>   혹은 서브넷을 결정하려면 먼저 호스트나 라우터에서 각 인터페이스를 분리하고 고립된 네트워크를 만든다. 이 고립된 네트워크의 종단점은 인터페이스의 끝이 된다. 이렇게 고립된 네트워크 각각을 서브넷이라고 부른다 
>
> - IP 주소체계는 이 서브넷에 223.1.1.0/24라는 주소를 할당해주는데, 여기서 /24는 **서브넷 마스크**라 부르며 32비트 주소의 왼쪽 24비트가 서브넷 주소라는 것을 가리킨다.
>
>![image-20201105152530276](README.assets/image-20201105152530276.png)
>
>여기서 서브넷의 갯수는 3개인 것 같지만 서브넷은 호스트를 라우터 인터페이스에 연결하는 이더넷 세그먼트만을 의미하는 것이 아니라 라우터와 라우터 간의 인터페이스도 포함시킨다 따라서 6개의 고립된 서브넷을 얻는다.
...
>
>개인 주소를 사용하여 인터넷에 네트워크를 연결하려면 NAT(네트워크 주소 변환)( Network Address Translation (NAT).)을 사용하여 개인 주소를 공용 주소로 변환해야 한다.
...
>
>트라고 말한다. 이를 **프리픽스** 또는 네트워크 프리픽스라고 한다. (파랑 부분)
>
>![image-20201028153246020](README.assets/image-20201028153246020.png)
>
>호스트 넘버로 특정 호스트를 식별한다. 
>
>![image-20201105153614316](README.assets/image-20201105153614316.png)
...
>>
>>  - device가 부팅되면 동일 서브넷에 위치한 DHCP 서버를 찾기 위해 **DHCP discover message**를 이더넷 망에 브로드캐스팅한다. (IP dest:FF-FF-FF-FF-FF) 서브넷에는 여러 DHCP 서버가 존재하기 때문에 DHCP 제공 메세지로부터 가장 최적의 위치에 DHCP 서버를 선택한다. 
>>
>>    메세지 내의 목적지 IP주소를 255.255.255.255.255.portnumber 로 설정하며 위의 사진은 포트 번호가 67인 것이다. 그리고 출발지 IP주소는 0.0.0.0 으로 설정한다.
>>
>>2. DHCP offer (서버 제공)
>>
>>  - DHCP discover 메시지를 수신한 DHCP 서버는 **DHCP offer message** broadcast 또는 unicast로 클라이언트에게 전송한다. 여기에는 장치(클라이언트)에게 임대해 줄 IP 주소와 자신의 IP 주소가 포함되어 있다. 또 한 라이언트의 IP 주소, Subnet mask, Default gateway IP 주소, DNS 서버 IP 주소, LifeTime(IP 주소 임대 시간)도 포함하고 있다. yiaddrr  는 임시 IP를 나타내며 보낼때 마다 같이 보내는 이유는 확인하기 위해서이다. 또한 **라우터의 MAC address**도 알려준다.
>>
>>3. DHCP request (요청)
>>
...
>>
>>2,4번에서 DHCP를 보낼때 브로드캐스트 메시지 (Destination MAC = FF:FF:FF:FF:FF:FF) 혹은 유니캐스트일수 있으며 이는 단말이 보낸 DHCP Request 메시지 내의 Broadcast Flag=1이면 DHCP 서버는 DHCP Ack 메시지를 Broadcast로, Flag=0이면 Unicast로 보내게 된다.
>
>**DHCP: more than IP addresses**
>
>>DHCP는 서브넷에서 할당받은 IP 주소 외에도 다음 정보를 전달
>>
>>- 고객의 first-hop router의 주소 (MAC address)
>>- DNS 서버의 이름과 IP 주소
...
>>
>>- DHCP 서버는 클라이언트의 IP 주소, 클라이이언트의 first-hop router의 IP 주소, DNS 서버의 이름 및 IP 주소를 포함하는 DHCP ACK을 공식화한다
...
>
>NAT 가능한 라우터는 외부 세계로는 라우터처럼 보이지 않는다. 대신 NAT 라우터는 외부세계로는 하나의 IP주소를 갖는 하나의 장비로 동작한다. 
>
>implementation: NAT router must:
>
>- outgoing datagrams: 모든 나가는 데이터그램의 (출발지 IP 주소, 포트 번호)를 (NAT IP 주소, 새로운 포트 번호 -> 목적지 주소로 사용)로 대체한다.
>- 모든 (출발지 IP 주소, 포트 번호)->(NAT IP 주소, 새로운 포트 번호) 변환 쌍을 **NAT translation table**에 기록한다. (밑에 그림 참조)
...
>- **hop** **limit**: IPv4에서의 TTL필드랑 하는 역할 유사 
>
> IPv4로부터 변화
...
>- IPV4 에서의 명칭와 IPV6의 명칭
>
>![image-20201126221103680](README.assets/image-20201126221103680.png)
>
>
>
>**ICMP: Internet control message protocol** (ip랑 같은 계층)
>
...
>**Tunneling** (터널)
>
>>IPv4 라우터들이 IPv6 데이터그램을 IPv4의 데이터 필드에 넣어 운반한다.
>>
...
>
>**흐름 테이블**을 포함한다. (포워딩 테이블의 업데이트 버전이라고 생각하였다.)
>
...
>- 128.119.1.1 소스로부터 온 데이터는 다 드랍하라
...
>
>>1c는 게이트웨이 라우터라고 하며  1a,1b,1d는 내부 라우터 라고 한다. 
>
...
>
>> - **NEXT-HOP** : AS-PATH를 시작하는 라우터 인터페이스의 ip주소이다. 
>
>>   예를 들어 밑에 AS1에서 서브넷 x까지 갈때에 AS2 AS3 X, AS3 X는 각 2a,3d의 인터페이스 주소를 나타낸다.
>
>>   다음과 같은 특정 내부-AS라우터를 다음 홉AS로 나타냄
>
...
>>- AS2 정책에 따라 AS2 라우터 2a는 AS2, AS3, X 경로를 AS1 라우터 1c에 알린다 (eBGP를 통해)
>
...
>>4. 추가 기준
>
>#### BGP: achieving policy via advertisements(광고를 통한 정책)
>
...
>
>>  ![image-20201118154550284](README.assets/image-20201118154550284.png)
>
...
>>#### Open Flow
>
>>>OpenFlow는 제어 평면과 데이터 평면을 나누어 관리 하며, 서로 다른 스위치와 라우터의 플로우 테이블을 프로그래밍 할 수 있는 오픈 프로토콜을 통하여 소프트웨어 기반의 프로그래밍 가능한 네트워크 환경을 제공해준다.
>
>#### SDN 관점 : 데이터 평면(data plane) 스위치
>
>>![image-20201118160602011](README.assets/image-20201118160602011.png)
>
>#### 데이터 평면 스위치
>
...
>
>>#### SDN 관점 : 애플리케이션 컨트롤 혹은 네트워크 제어 응용
>
>>>- 네트워크-제어 앱
>
...
>>#### OpenFlow 프로토콜
>
>>>![image-20201118160811920](README.assets/image-20201118160811920.png)
>
>>>- 컨트롤러 간 동작
>
>>>- **메세지** 교환에 사용되는 TCP
...
>>>- 대칭 (기타)
>
>>>#### 컨트롤러에서 제어하는 메시지(controller-to-switch messages) ⬇
>
>>>>주요 컨트롤러 - 스위치 메시지
>
>>>>- features(기능) : 컨트롤러 쿼리 기능 전환, 응답전환
>>>>- configure(구성) : 컨트롤러 쿼리 및 스위치 구성 변수 설정
>>>>- modify-state(상태수정) : OpenFlow 테이블에서 흐름 항목 추가, 삭제, 수정
>>>>- packet-out(패킷 전송) : 컨트롤러는 이 패킷을 특정 스위치 포트에서 보낼 수 있음
>
>>>#### 컨트롤러 간 메시지(switch-to-controller messages) ⬆
>
>>>>중요한 부분 : 컨트롤러 간 메시지
>
>>>>packet-in(패킷전달) : 패킷(및 그 제어)을 컨트롤러로 전송하며 컨트롤러로 부터 패킷출력 메세지를 관찰
>
>>>>flow-removed(흐름 제거) : 스위치에서 삭제된 flow table 항목
>
>>>>port status(포트 상태 ): 컨트롤러에 포트 변경을 알림
>
>>>>다행하게도, 네트워크 운영자는 OpenFlow메시지를 직접 생성/전송하여 스위치를 프로그래밍하지 않는다. 대신 컨트롤러에서 더 높은 수준의 추상화를 사용함
>
>#### SDN : 컨트롤 / 데이터 플레인 상호 작용 예제 
>
>(기말 전체네트워크 순서HTTP, DHCP, DNS, TCP, RIP ARP)
>
>HTTP(HyperText transfer Protocol)
>
>DHCP(Dynamic Host Configuration Protocol)
...
>
>ARP(Address Resolution Protocol) ARQ(Automatic repeat requst) 랑 다름!!
>
...
>
>ISP의 네트워크와 구글의 네트워크 사이에 있는 도메인 내 링크를 통해 전달되는 패킷들은 BGP 즉 RIP에 따라 통제된다.
>
>이후 HTTP통신을 하게 된다. 
>
>DHCP->DNS->TCP->RIP->HTTP
>
>DHCP ->ethernet -> ARP -> DNS-> TCP -> HTTP
>
>>![image-20201120174921432](README.assets/image-20201120174921432.png)
>
>>>스위치 s1,s2가 단절되었다고 가정하고
>
...
>
>꼭 오류 상태만을 알리기 위해서는 아닌다. 
>
>#### 추적(Traceroute) 및 ICMP
>
>![image-20201120181736496](README.assets/image-20201120181736496.png)
>
...
>
>  해당 라우터는 데이터그램의 TTL이 방금 만료된 것을 알고  데이터그램을 삭제하고 원본 ICMP 메시지(유형 11, 코드 0)를 출발지에 보낸다. 
>
>  ICMP 메시지는 라우터 및 IP주소가 포함되어 있기때문에 이 메시지가 출발지에 도착하면 출발지는 타이머로부터 왕복시간 , ICMP 메시지로부터 n번째 라우터의 주소와 이름을 획득한다. 
>
...
>
>대상이 ICMP "포트연결 불가" 메시지를  보낸 곳에 반환(유형3, 코드3)
>
>출발지는 ICMP메세지를 받고 소스 스톱
...
>
>> QoS : Quality of service, 서비스의 품질 (bandwidth) SLA : SI업체, Ni업체에게 정부가 요구하는 서비스 품질(어느 정도의 서비스를 요구하는)
>
>책 정의: 네트워크 관리는 적정한 비용으로 실시간, 운용성능, 서비스 품질 등의 요구사항을 만족시키기 위하여 네트워크와 구성요소 자원들을 감시, 테스트, 폴링 , 설정, 분석, 평가 , 제어하는 하드웨어, 소프트웨어, 인간요소 등을 배치하고, 통합, 조정하는 것이다. 
>
>#### 네트워크 관리를 위한 인프라
>
>>![image-20201120184244289](README.assets/image-20201120184244289.png)
>
>>- 관리서버에는 네트워크의 동작을 제어하기 위한 작업이 시작되며 네트워크 관리자는 네트워크 장치들과 상호작용한다. 
>>- 관리 장치에는 데이터가 MIB (**Management Information Base**)로 수집되는 관리 대상 개체가 포함
>>- 위에 보이는  data가 MIB를 나타냄 
>>- Managed device(피 관리장치) = host, sensor 등 대상이 점점 늘어남
>>- Data Parameter를 가지고 관리
>>- **네트워크 관리 프로토콜**: 관리서버와 피관리 장치들 사이에서 동작하면서 관리 서버가 피관리 장치의 상태에 대해 질의하고 에이전트를 통해 간접적으로 피관리 장치에 행동을 취할 수 있도록 한다.
>>- 용어들에 대한 정의를 명확히 해야 표준을 정할 수 있음
>
>#### Simple Network Management Protocol (SNMP)
>
>![image-20201120185925304](README.assets/image-20201120185925304.png)
>
>
>![image-20201120185936439](README.assets/image-20201120185936439.png)
>
>
>![image-20201120185955201](README.assets/image-20201120185955201.png)
>
>
...
>  - 무선 링크
>  - LAN
>- 계층 2 패킷 : **프레임**, 데이터 그램 캡슐화 즉 프레그먼트-> 데이터그램 -> 프레임
>
...
>  - IP주소와는 다르다. 
>- **인접한 노드 사이의 안정적인 전달** **(reliable delivery between adjacent nodes)**
>
>  우리는 이미 이것을 어떻게 하는지 배웠음(Chapter 03 TCP)
>
>  거의 사용되지 않은 비트 오류 링크 (광섬유, 일부 꼬임 쌍 에서는 불필요한 오버헤드)
>
...
> - 트랜스포트, 네트워크의 오류 검출(checksum) 이 있듯이 링크 계층의 오류 검출은 일반적으로 더 복잡하며 하드웨어로 구현된다. 
...
>![image-20201127172108212](README.assets/image-20201127172108212.png)
>
>- 각 호스트마다 구현댐 (종단에서)
>- "어댑터"(**Network Interface Card, NIC**) (card 대신 controller도 가능)또는 칩(주로 메인보드에 같이 구현)에 구현된 링크 계층
>
>  이더넷 카드, 802.11 카드; 이더넷 칩셋
>
>  링크, 물리계층 구현
>- 호스트의 시스템 버스에 연결
>- 하드웨어, 소프트웨어, 펌웨어의 조합하여 구현된다. 

### 통신어뎁터 (Adaptors communicating)

>![image-20201127172229659](README.assets/image-20201127172229659.png)
>
>- 보내는 쪽
>
>  프레임에 데이터그램 캡슐화
>
>  오류검사비트, rdt(real data Transport), 흐름제어 등을 추가
>- 받는 쪽
>
...
> ![image-20201127172311129](README.assets/image-20201127172311129.png)
>
> - EDC = 오류감지 및 수정비트() (중복성)
>
...
>
> - 수신자는 EDC와 EDC`이 같은지 확인한다. 
>
> - 100% 신뢰할 수 없는 오류감지이다.
>
>   - 프로토콜이 약간의 오차를 발생시킬 수 있지만 드물다. (가능 하긴 한 것)
>   - EDC가 커지면 검출 및 보정이 향상
>
> 더 정밀한 검사가 가능하지만 오버헤드가 발생한다.
>
> 전송되는 데이터의 오류를 검출하기 위한 세가지 기술 
>
> >**패리티 검사**( 오류 검출 및 정정의 기본 개념을 보기 위함) 
> >
> >**체크섬**( 대체로 트랜스포트 계층에서 사용)
> >
> >**순환중복검사**(cyclic redundancy check 대체로 어뎁의 링크계층에서 사용)이다. 

### Parity Checking( parity 검사)

>- single bit parity : 단일 비트 오류 감지
>
>  ![image-20201127172439237](README.assets/image-20201127172439237.png)
>
>- two-dimensional bit parity : 2차원 비트 패리티 (column, row parity)
>
>  밑에 사진에서 보이는것과 같이 틀린부분을 찾아내 수정이 가능하다.  또 한 오류 2개도 잡을수 있다.
>
>![image-20201127172448476](README.assets/image-20201127172448476.png)
...
>  - 아니오 : 오류 감지
>
>체크섬 방법은 상대적으로 패킷 오버헤드가 적다. TCP와 UDP 체크섬은 16비트만 사용한다.
>
>CRC랑 비교하면 오류 검출면이 떨어지는데 왜 트랜스 포트 계층은 체크섬을 사용할까?
>
>트랜스 포트 계층은 운영체제의 일부로서 소프트웨어에서 구현된다. 따라서 간단하고 빠른 오류 검출 기법이 필요로 하는 반면에 링크계층은 어뎁안의 전용 하드웨어에서 구현되므로 더 복잡한 CRC연산을 수행할 수 있다. 

#### Cyclic redundancy check (순환 중복 검사) (CRC)

>![image-20201127172817927](README.assets/image-20201127172817927.png)
>
>보다 강력한 오류 감지 코드화
>
>데이터 비트, D 를 이진수로 보기
...
>r+ 1비트 패턴(제너레이터), G 선택
>
>목표: 다음과 같이 R CRC 비트, R , 를 선택한다.
>
>><D,R>은 정확히 G로 나누어져 있다(모듈로 2).
>>
>>수신기가 G를 알고, <D,R>을 G로 나눈다. 0이 아닌 나머지:오류 감지!
>>
>>r+ 1비트보다 작은 모든 버스트 오류를 감지할 수 있음
>
>실무에 널리 사용됨(이더넷, 802.11 WiFi, ATM)
>
>example
>
>G의 자릿수-1 만큼 D의 크기를 0으로 매꾸고 
>
>빼기 연산이 아닌 XOR연산이며 반환값은 기본 101110 000 에서 추가된 자릿값을
>
>R로 매꾸고 반환한다. 101110011 이고 수신측에서 이것을 다시 1001 로 나누면 나머지값이 0이다.
>
>![image-20201127172940581](README.assets/image-20201127172940581.png)
...
>
>  채널을 더 작은 '조각' (**타임슬롯, 주파수, 코드**) TDMA,FDMA,CDMA 로 나뉨
>
...
>![image-20201127173354405](README.assets/image-20201127173354405.png)
>
...
>
>예를 들어 칵테일 파이에서 파티 참석자 한 명이말하고 다른 사람 기다리고 다른사람 말하고 느낌
>
>TDM은 공정하지만 노드 전송률이 평균 R/N으로 낮고 노드가 전송 순서상 자신의 차례를 항상 기다려야 한다. 

### 채널 분할 MAC 프로토콜 : FDMA 

>![image-20201127173442100](README.assets/image-20201127173442100.png)
>
...
>TDMA같은 장단점을 가지고 있다. 
>
>CDMA는 여러 노드들이 동시에 전송할 수 있고 다른 노드들에 의해서 전송이 간섭되더라도 각 수신자들이 송신자의 인코딩된 데이터 비트를 정확하게 수신 할 수 있다.  즉 CDMA는 다중 접속 채널 사용자에게 할당될 수 있다는 것이다.

### Random access protocols

>- 사전에 coordination(조정) 하는것은 없고, 아무때나 보내고 싶을 때 보냄
>
>   충돌이 생기면 충돌없이 전송될떄까지 자신의 프레임을 계속 전송 
>
>   그 즉시 보내는 것이 아닌 재전송하기전에 랜덤 지연 시간 동안 기다린다. 
>
>- 노드가 보낼 패킷이 있을 때
>
>  풀 채널 데이터 속도 R로 송신
>
>  노드 사이의 우선순위 조정이 이루어지지 않음
>
...
>
>![image-20201127173628884](README.assets/image-20201127173628884.png)
>
>그림에서 첫 슬롯에서 노드 1,2,3이 충돌하며 노드2는 4번쨰, 노드1은 8번째, 노드3은 9번째 슬롯에서 성공한다. 
>
>- 장점
>
...
>  클럭(clock) 동기화
>
>  활성슬롯이 많으면 일부 슬롯이 충돌로 인해 결과적으로 낭비되고 확률적인 전송 정책 때문에 전송을 억제하는 경우 일부 슬롯이 빈다. 낭비 되지않는 슬롯은 정확히 한 노드만 전송하는 슬롯이다.
>
...
>프레임이 도착하면 노드는 즉시 그 프레임 전체를 브로트캐스트 채널로 전송한다. 만일 전송된 프레임이 하나 이상의 다른 전송과 충돌하면, 노드는 확률p로 그 프레임을 즉시 재전송한다. 즉시 재전송을 하지 않으면
>
>노드는 프레임 전송시간 동안 기다린다. 따라서 노드는 기다리고 나서 그 프레임을 확률 p로 전송하거나 확률 1-p로 또 다른 프레임 시간동안 기다린다. 
>
>- 단순, 동기화 없음
>- 프레임이 처음 드랍될 때
>
...
>
>![image-20201127173924494](README.assets/image-20201127173924494.png)
>
>P(주어진 노드에 의한 성공) = P(노드 전송) =P(다른 노드는 [t0-1 ,t 0 ]에서 전송되지 않음)
>=P(다른 노드는 [t0-1 ,t 0 ]에서 전송되지 않음)
>
>=p * (1-p)^(n-1) *(1-p)^(n-1)
>
>=p* (1-p)^2(n-1)
...
>슬롯 알로하와 순수 알로하는 노드가 전송 여부를 결정하는것은 브로드 캐스트 채널에 접속되어 있는 노드를 신경 안쓴다. 예를 들어 칵테일 바에서 다른사람이 말하고 있어도 자기 의견을 계속 내는 것이다. 
>
>- CSMA : 전송 전에 듣기
>
> 채널 감지 유휴 : 전체 프레임 전송
>
>- 만약 채널이 사용중임을 감지하면 전송을 지연시킨다. 채널을 듣는 것을 carrier sensing 캐리어 감지라고 한다. 
>
>- 다른 채널이 동시에 말하기 시작하면 전송을 지연시키고 랜덤 시간동안 기다린 후 유휴시 감지 및 전송과정을 반복한다. 
>
>- 위의 두 규칙을 CSMA 와 CSMA/CD 라고 한다. 
>
...
>#### 충돌
>
>>![image-20201127214802320](README.assets/image-20201127214802320.png)
>
>>- 위의 사진에서 B는 t0시간때에 아무도 전송하지 않아서 전송을 시작하며 전송한 비트들이 브로드캐스트 매체를 따라 양방향으로 전파된다. 하지만 시간 t1에서는 b의 전파가 아직 오는것을 눈치채지 못하고 D가 전송을 시작하여 간섭이 시작된다. 이런 브로드캐스트 채널의 종단간의 **채널전파 지연**( channel propagation delay) 이 CSMA의 성능을 결정한다.  위의 그림은 충돌 검출을 수행하지 않는 모습이다. 
...
>#### CSMA/CD (충돌 감지)
>
>>![image-20201127214836429](README.assets/image-20201127214836429.png)
>
>>​																							충돌 검출 시간
>
>>랜덤 시간동안 기다리는 이유는 만일 두 노드가 동시에 프레임을 전송하고 고정시간동안 기다리게 된다면 계속해서 충돌하기 때문이다. 
>
...
>>2. NIC가 채널 유휴(channel idle)(아무도 안씀)를 감지하면, 프레임 전송을 시작하고 NIC가 사용중인 채널을 감지하면 채널이 유휴상태가 될때까지 기다린다음 전송
>>3. NIC가 다른 전송을 감지하지 않고 전체 프레임을 전송할 경우 NIC가 프레임으로 이루어짐
>>4. NIC가 전송신호를 전송하는 동안 다른 전송을 감지하는 경우, 중단하고 신호를 전송
>>5. 중단 후, NIC는 이진수(지수)백업을 시작 (**binary (exponential) backoff**)
>>  - 중단 후 NIC는 K (0,1,2, … , 2^m-1) 에서 무작위로 선택하고 NIC가 K*512 bit 동안 대기 후 2단계로 리턴 (m은 충돌횟수)
>>  - 더 많은 충돌이 발생한다면 더 긴 백업 간격을 가짐
>>  - 따라서 새 프레임을 준비할때 최근 충돌을 고려하지 않기때문에 다른노드는 지수적 백오프를 수행하고 있을때 새로운 프레임을 가진 노드가 먼저 성공적으로 전송할 수 있다. 
>>  - 이더넷에서 백업을 하는 부분이 알고리즘으로 구현되어 있음
>
>#### CSMA/CD 장점(기말)
>
>>![image-20201127222350988](README.assets/image-20201127222350988.png)
>
>>- T-prop = LAN의 2 노드 간 최대 지연시간
>>- t-trans = 최대 크기의 프레임 전송시간
>>- 효율성은 1에 근사
>
>>  t-prop가 0이 될때 : 이유는 전파 지연이 0이되면 충돌한 노드들이 채널을 낭비하지 않고 즉시 취소하기 때문이다. 
>
>>  t-trans가 무한대로 이동함에 따라 : 혹은 trans가 아주크면 효율은 1이다 .그 이유는  프레임이 채널을 한 번 차지하면 아주 오랫동안 채널을 사용할 것이다. 따라서 채널은 모든 시간 동안 유용하게 쓰이게 된다. 
>>
>>  - **ALOHA보다 우수한 성능을 제공, 간단하고 저렴하며 분산되어있음**

### Taking turns MAC protocols (교대 MAC 프로토콜 기말)
...
>  두 세계의 최고를 찾으십시오!
>
>![image-20201127222924285](README.assets/gif4.gif)
>
>##### 폴링(palling) - 순서대로 보내주는 것
>
>- 노드중에 마스터 노드를 정한후 각 노드를 라운드 로빈 방식으로 폴링한다. 
>
...
>  폴링 오버헤드 
>
>  지연: 폴링 지연(노드가 전송할 수 있음을 알리는데 걸리는 시간) 즉 활성노드는 Rbps보다 작은 전송률로 전송하게 된다. 
>
>  단일 장애 지점(마스터) : 마스터 노드가 고장 나면 전체 채널이 동작하지 못하게 된다. 
>
>- 802.15 프로토콜 및 블루틋 프로토콜은 폴링 프로토콜이다. 
>
>![image-20201127222937041](README.assets/gif5.gif)
>
>- **token-passing protocol: 순서를 가지고 token으로 passing 하는 방법 **
>
>노드1이 노드2에게 토큰을 전송 노드2가 노드3에게 토큰을 전송하는 방식이다. 
>
>**(Token이 돌면서 passing 하거나 data를 전송하거나)**
>
...
>  지연
>
>  단일 장애 지점(토큰) : 노드 하나가 실패하거나 노드가 잘못되서 토큰을 놓아주지 않으면 토큰이 다시 돌수 있도록 하는 회복 절차가 있어야 한다. 

### Cable access network (케이블 접근 네트워크)

>케이블 접속망은 랜덤 접속, 순번, 채널 분할 프로토콜 각각에 대해 보여준다. 
>
>![image-20201127223016143](README.assets/image-20201127223016143.png)
>
>- 다운스트림 업스트림으로 나누기 위해 FDM을 사용한다. 
...
>
>![image-20201127223040396](README.assets/image-20201127223040396.png)
>
>- t1에서 화살표 끝까지 : 미니 슬롯 요청 프레임을 포함하는 미니 슬릇
>- 화살표에서 t2까지: 케이블 모뎀 상향 데이터 프레임을 포함하는 할당된 미니 슬롯
>- **DOCSIS** : **data over cable service interface spec** (케이블 모뎀 종단 시스템)
>- 업스트림, 다운스트림 주파수 채널을 통한 FDM을 사용한다. 
>- 상향채널, 하향 채널 둘다 브로드케스트 채널이다. 
...
>- LAN이란 동일한 IP대역과 동일한 서브넷 마스크를 사용하며 ARP가 닿는 모든 범위를 말한다. 
>- 스위치 근거리 네트워크(switched local area network) 에서는 링크 계층에서 작동하기 때문에 링크 계층 프레임을 교환하고 스위치들로 구성된 망에서 경로를 결정하는데 RIP나 OSPF와 같은 라우팅 알고리즘을 사용하지 않는다. 
>
> 실제로 링크 계층 주소를 가진 것은 호스트나 라우터가 아닌 호스트와 라우터의 어댑터(네트워크 인터페이스)이다.
>
> 다수의 네트워크 인터페이스를 가지고 있으면 여러 개의 링크 계층 주소를 가진다.
>
> 여기서 **링크 계층 스위치**는 호스트나 라우터를 연결해 주는 인터페이스에 링크 계층 주소를 할당 받지 않는다. 
>
>- 맥주소는 총 16진수 12자리 이므로 **2^48**으로 나타낼 수 있다. 
>
>- 앞에서 배운것 처럼 호스트와 라우터는 네트워크 계층 주소를 가지면서 링크 계층의 주소도 가진다. 그 이유를 뒤에서 알아본다. 
>
>- ARP는 네트워크, 링크 주소를 포함하기 때문에 링크 계층과 네트워크 계층의 경계에 있는 프로토콜이다. 
>
>- 32비트 IP 주소
>
...
>
> 기능 : **하나의 인터페이스에서 다른 물리적으로 연결된 인터페이스로 프레임을 가져오기위해 '로컬'로 사용됨** **(IP 주소 지정 의미에서 동일한 네트워크)**
>
...
>
>![image-20201127230456757](README.assets/image-20201127230456757.png)
>
>LAN 상의 각 어뎁터에는 **고유한 LAN** 주소가 있음
>
>
>
...
>- ARP: IP주소를 링크 계층 주소로 변환하는 역할을 한다. 
>- 어떻게 IP주소를 알고 인터페이스의 MAC주소를 결정하는가?
>
>![image-20201127230639293](README.assets/image-20201127230639293.png)
>
...
>>
>>* A는 B의 IP주소를 포함하는 **ARP 쿼리 패킷**을브로드캐스트함
>>
...
>>
>>A의 MAC주소 (유니캐스트)로 전송된 프레임(브로드캐스트 아님!)
>>
...
>![image-20201127230947112](README.assets/image-20201127230947112.png)
>
...
>
>![image-20201127231001347](README.assets/gif1.gif)
>
...
>![image-20201127231135421](README.assets/image-20201127231135421.png)
>
...
>
>>![image-20201127231150313](README.assets/image-20201127231150313.png)
>
>>- 버스
>>
>>  : 90년대 중반까지 인기 - 동축(coaxial) 케이블
>>
...
>#### 이더넷 프레임 구조 (기말 1문제)
>
>>![image-20201127231214637](README.assets/image-20201127231214637.png)
>
>>- Preamble(8byte), source address(6byte), dest address(6byte), IP(20byte), TCP(20byte), data(46~1460Byte)
...
>>- **유형(type 2바이트)** : 상위 계층 프로토콜을 나타냄 (대부분 IP이지만 Novell IPX, AppleTalk와 같은 다른 프로토콜도 가능)
>>- CRC (4바이트) : 수신기에서 순환 중복 검사
>>
>>  오류 감지 : 프레임이 떨어짐
>
>#### 이더넷 : 비 신뢰, 비 연결
...
>>
>>  CRC 검사에서 실패해도 부정 확인 응답을 보내지 않고 그냥 프레임을 폐기 . 보낸입장에서는 통과했는지 알 수 없다. 
>>
>> - 손실된 프레임이 더 높은 계층(예:TCP)을 사용하는 경우에만 손실된 프레임의 데이터가 복구되며, 그렇지 않을경우 손실됨
>>
>>   UDP 같은 경우는 데이터 손실로 인해 손해 볼 수 있다. 
>>
>>- 이더넷 MAC 프로토콜 : **이진 백 오프**가있는 슬롯된 **CSMA / CD**

#### 802.3 802.3 Ethernet standards: link & physical layers 

#### (이더넷 표준 : 링크 및 물리적 레이어)

>![image-20201127231809927](README.assets/image-20201127231809927.png)
>
>- 다양한 이더넷 표준
>
...
> 저장, 전달 이더넷 프레임
>
> 라우터 출력 인터페이스가 데이터그램에 대한 버퍼를 가지고 있듯이 스위치 출력 인터페이스도 버퍼를 가지고 
>
> 프레임의 MAC주소를 검사하고 프레임이 세그먼트에 전달될 때 프레임을 일대일 링크로 전달한다. 프레임이 세그먼트에 액세스 하려면 CSMA/SSCD를 사용한다.
>
>- **transparent**(투명한)
>
...
>
>>![image-20201127232056918](README.assets/image-20201127232056918.png)
>
>>- 호스트는 전용 스위치를 직접 연결
>
>>- 패킷들 버퍼 전환
>
>>- 각 수신링크에서 사용되는 이더넷 프로토콜이지만 충돌은 없기 때문에  **전이중** 이다.
>
>>  각 링크는 자체 충돌 도메인
>
>>- **스위칭** : A-to-A' 및 B-to-B' 는 충돌없이 동시에 전송할 수 있다.
>
>>  테이블이 맥주소에 대한 엔트리가 없으면 이 스위치는 프레임의 복사본을 인터페이스 x를 제외한 모든 인터페이스에게 전달한다.
>
>>  인터페이스 1에서 자기자신 맥주소로 보내는 주소는 그 프레임을 폐기한다.
>
>>  만약 동일한 Lan에서 보내는 것은 해당하는 인터페이스로 보낸다. 
>
>#### 스위치 포워딩 테이블(forwarding table)
>
>>- Q. 스위치가 A' 를 인터페이스 4를 통해 연결할 수 있고, B'를 인터페이스 5를 통해 연결할 수 있음을 어떻게 알 수 있는가?
>>- A. 각 스위치는스위치 테이블을 가지고 있고, 각 항목은 다음과 같다
>
...
>### 스위치 : Self-learning
>
>>>![image-20201127232116620](README.assets/gif2.gif)
>
>>>- A가 출발지, A'가 도착지
>>>- 스위치는 인터페이스를 통해 어떤 호스트에 도달할 수 있는지를 학습
>>> - 프레임 수신시 스위치가 송신자의 "학습" 위치 : 수신 LAN 세그먼트
>>> - 스위치 테이블에 보낸 사람/위치 쌍을 기록
>>> - 일정시간(aging time)이 지난후에도 스위치가 해당 주소를 출발지 주소로 하는 프레임을 수신하지 못하면 테이블에 이 주소를 삭제
>
>#### Switch: frame filtering/forwarding (스위치 : 프레임 필터링 / 전달)
>
...
>
>>![image-20201127232116620](README.assets/gif3.gif)
>
>>- 프레임 대상, A' 알 수 없는 위치 : 홍수(flood)-> 랜에 있는 호스트에게 전부 뿌린다.
...
>
>>![image-20201127232507303](README.assets/image-20201127232507303.png)
>
...
### Self-learning multi-switch example(자가 학습 멀티 스위치 예제)

>![image-20201127232536099](README.assets/image-20201127232536099.png)
>
...
>![image-20201127232601092](README.assets/image-20201127232601092.png)

## 스위치 vs 라우터

>![image-20201127232657611](README.assets/image-20201127232657611.png)
>
...
>- **스위치(switches)** : flood,self-learning, MAC 주소를 사용하여 전달 테이블 학습

### 허브,스위치, 라우터 비교

>허브는 여러 대의 컴퓨터를 연결해서 네트워크를 만들어주는 장치로서, `데이터 패킷을 받으면 연결된 모든 장치들에게 모두 보낸다.`
>
>공유기는 ISP(Internet Service Provider)업체에서는 IP를 제공하고, 소비자는 해당 IP를 구매해서 사용하여 여러대의 접속을 지원해준다.
>
>|                  | 허브   | 라우터 | 스위치 |
>| :--------------- | ------ | ------ | ------ |
>| 트래픽 경리      | 아니요 | 예     | 예     |
...
>| 최적 라우팅      | 아니요 | 예     | 아니요 |
>
>스위치는 2계층까지만 패킷을 처리해서 높은 패킷 여과 및 전달률을 가진다. 하지만 브로드캐스트 트래픽 폭주에 대비한 방안은 없다.
>
>라우터는 패킷을 스패닝 트리로 제한하지 않고 최상의 경로를 사용할 수 있고 트래픽 폭주를 방지할 방화벽이 있지만
>
>플러그 앤 플레이가 아닌것처럼 느리다는 단점이 있다.

## VLANs : 동기부여 (motivation)

>![image-20201215211750503](README.assets/image-20201215211750503.png)
>
...
>
>VLAN기능을 지원하는 스위치는 단일 물리적 LAN인프라를 통해 여러 가상 LAN을 정의하도록 구성할 수 있다.
>
>**포트 기반 VLAN** : 스위치 포트를 그룹화하여 (스위치 관리 소프트웨어로) 단일 물리적 스위치
>
>여러 개의 가상 스위치로 작동
>
>![image-20201127232805445](README.assets/image-20201127232805445.png)
>
>- **traffic isolation(트래픽 격리)** : 포트 1-8 사이의 프레임은 포트 1-8에만 도달 할 수 있음
>
> - 또한 스위치 포트가 아닌 엔드 포인트의 MAC 주소를 기반으로 VLAN을 정의 할 수 있음
>
>- **dynamic membership**(**동적 멤버십**) : VLAN간에 포트를 동적으로 할당 할 수 있습니다.
>
>- **VLANS 간 전송**: 라우팅을 통해 완료 (별도의 스위치와 동일)
>
> - 실제로 벤더들은 결합 된 스위치와 라우터를 판매
>
>   ![image-20201215215028213](README.assets/image-20201215215028213.png)
>
>   위의 사진처럼 EE학과에서 CS학과로 트래픽을 보내려면 외부의 라우터에 연결하면 된다.

### VLANS spanning multiple switches

>![image-20201127233124336](README.assets/image-20201127233124336.png)
>
>**(trunk port) 트렁크 포트** : 여러 물리적 스위치를 통해 정의 된 VLANS간에 프레임 전송
>
>- 스위치간에 VLAN 내에서 전달되는 프레임은 바닐라(vanilla) 802.1 프레임 X (VLAN ID 정보를 전달해야 함)
>
>- 802.1q 프로토콜은 트렁크 포트간에 전달 된 프레임에 대한 추가 헤더 필드를 추가 / 제거
>
>  스위치가 어떻게 트렁크 포트로 온 프레임인지 어떤 VLan에 속한지 알기 위해서 
>
>  **VLAN tag**를 헤더에 넣는다.

### 802.1Q VLAN frame format

>![image-20201127234304248](README.assets/image-20201127234304248.png)

### Multiprotocol label switching (MPLS) (멀티 프로토콜 라벨 스위칭)

>![image-20201127234359503](README.assets/image-20201127234359503.png)
>
...
### MPLS capable routers (MPLS 지원 라우터)
...
>![image-20201127234501885](README.assets/image-20201127234501885.png)
>
>- IP 라우팅(routing) : 목적지 주소만으로 결정된 목적지까지의 경로
>
>![image-20201127234517561](README.assets/image-20201127234517561.png)
>
...
>![image-20201127234549954](README.assets/image-20201127234549954.png)
>
...
>![image-20201127234606807](README.assets/image-20201127234606807.png)
>
>IP라우팅 프로토콜은 A로의 최소비용경로만 정하지만 
>MPLS는 IP라우팅 프로토콜에 불가능한 패킷을 전달한다. 
>
>또 한 MPLS는 VPN를 구현하는데 사용한다. 
...
>빨 -> 녹색 -> 파랑
>
>데이터 센터의 호스트는 피자 박스 모양의 블레이드 적재하는 공간을 랙이라고 한다. 
>
>랙 맨 위에는 TOR스위치 가 있고 이는 곧 다른 스위치랑 연결된다. 
>
>![image-20201127234705562](README.assets/image-20201127234705562.png)
>
...
>
>![image-20201127234743559](README.assets/image-20201127234743559.png)
>
...
- 웹 접속시 순서 (첫번째, 맨끝)
- Subnet 개념
- Subnet 계산 (기준 IP주소를 써라) 2문제 (연습문제 비슷한 형태로 나오니 완벽하게 알고 갈 것)
  - 그 기준 IP를 정하라는 것도 나옴 (192.168.0.100/24 회사에서 4개로 나눌것인데 그 기준IP가 무엇인가)
- IPv4, IPv6 헤더를 연계하는 문제 (예: IPv4의 checkSum은 IPv6에서 어떤것이 담당하는데 그 해당 필드가 몇 Bit인가?)
- Init Goal 에서 단어문제 하나 나옴 (설명 후 단어 맞추기), 설명후 단어맞추는거 하나 더 나오는데 영어로 나오므로 PPT 잘 보고 갈 것
- Wireshark IP 부분 TCP 연습문제 부분 기본적으로 어떻게 나오는지 이해할것