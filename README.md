...
>>>
>>옛날방식으로는 IP address 도착지를 기반으로 포워딩 했지만
>>>>
>>요즘은 헤더 필드 값에 근거하여 포워딩을 한다.
>>>
>>**match plus action**
>>>
>>match - 목적지 IP 주소를 찾음
...
>>
>>  >1. 물리 및 링크 계층 프로세싱 해야함
>>  >2. 패킷의 버전번호, 체크섬 및 TTL 필드를 확인하고 두 필드를 다시 사용한다.
>>  >3. 네트워크 관리에 사용되는 카운터(수신 된 데이터 그램 수) 를 업데이트해야 한다.
>>
...
>>
>> 입력 포트에 있는 서로 다른 2개의 패킷이 같은 출력 포트(빨간색)을 향해 간다. 이 중 한 패킷은 차단되고 입력 큐에서 기다려야 한다. 혹은두번째 사진처럼 앞의 패킷이 움직이지 않아서 기다리는 경우도 있다. 
>>
